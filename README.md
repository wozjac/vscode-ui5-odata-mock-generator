...
Additional options influecing data generation can be specified in *.rules.json* file. This file is by default searched in the project root, but it can be changed using setting `mockRulesConfigFilePath`. The JSON provided in this file corresponds to the  [*rules* property](https://wozjac.github.io/omg-odata-mock-generator/ODataMockGenerator.html).
...
For some values it make sense to make them dependent on other values. This can be achieved wiin the following way:
```javascript
{
  "predefined": {
    "Entity": {
